...
*   **URL:** `GET /rvm-ui/{uuid}?signature={signature}`
*   **Controller:** `App\Http\Controllers\Dashboard\KioskController@index`
*   **Fungsi:**
    *   Memvalidasi apakah `uuid` terdaftar dan aktif.
    *   Memvalidasi apakah `signature` valid.
...
UI hanya memiliki satu URL utama (`/rvm-ui/{uuid}?signature={signature}`), konten berubah dinamis berdasarkan State Vue.js:
...
### **Routes (Web)**
*   `GET /rvm-ui/{uuid}?signature={signature}`: Halaman utama Kiosk. Membutuhkan validasi UUID mesin yang valid dan signature yang valid.
...
*   Implementasi logika filtering log berdasarkan `uuid` di `LogController`.
...
├── routes/
│   ├── web.php                              # Route: /rvm-ui/{uuid}?signature={signature}
